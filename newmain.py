...
from models.rnn_models import VanillaRNN, SRNN_Softmax, \
    SRNN_Softmax_Temperature, SRNN_GumbelSoftmax
from models.lstm_models import VanillaLSTM, SLSTM_Softmax, \
    SLSTM_Softmax_Temperature, SLSTM_GumbelSoftmax
# Baby Neural Turing Machine (Baby-NTM)
from models.ntm_models import BNTM_Softmax, BNTM_SoftmaxTemperature, \
    BNTM_GumbelSoftmax
...
# GPU/CPU Check
device = torch.device(
    "cuda:0" if torch.cuda.is_available() else "cpu")  ## GPU stuff
print(device)
...
def get_args():
    ...
    parser.add_argument('--num_par', default=2, type=int,
                        help='number of parentheses pairs')
    parser.add_argument('--p_val', default=0.5, type=float,
                        help='p value for the PCFG for Dyck-n')
    parser.add_argument('--q_val', default=0.25, type=float,
                        help='q value for the PCFG for Dyck-n')
    # Training and test corpura parameters
    parser.add_argument('--training_window', default=[2, 50], type=int,
                        help='training corpus window')
    parser.add_argument('--training_size', default=5000, type=int,
                        help='number of training samples')
    parser.add_argument('--test_size', default=1000, type=int,
                        help='number of test samples')
    # Model choice
    parser.add_argument('--model_name', type=str, default='stack_rnn_gumbel_softmax',
                        choices=['stack_rnn_softmax',
                                 'stack_rnn_gumbel_softmax',
                                 ],
                        help='model choice')
    ...
    # Stack/Memory size
    parser.add_argument('--memory_size', default=104, type=int,
                        help='memory/stack size')
    parser.add_argument('--memory_dim', default=5, type=int,
                        help='memory/stack dimension')
    # Temperature + Annealing rate
    parser.add_argument('--temp_min', default=0.5, type=float,
                        help='minimum temperature value')
    parser.add_argument('--anneal_rate', default=0.0001, type=float,
                        help='anneal rate')
    ...
    parser.add_argument('--save_path', default='', type=str,
                        help='where you want to save the model')
...
def choose_model(model_name, n_hidden, vocab_size, n_layers, memory_size,
                 memory_dim):
    if model_name == 'stack_rnn_softmax':
        model = SRNN_Softmax(n_hidden, vocab_size, vocab_size, n_layers,
                             memory_size, memory_dim)
    elif model_name == 'stack_rnn_gumbel_softmax':
        model = SRNN_GumbelSoftmax(n_hidden, vocab_size, vocab_size, n_layers,
                                   memory_size, memory_dim)
    else:
...
def main():
    ...
    # training_input, training_output, st = Dyck.training_set_generator(
    #     TRAINING_SIZE, MIN_SIZE, MAX_SIZE)
    # test_input, test_output, st2 = Dyck.training_set_generator(TEST_SIZE,
    #                                                            MAX_SIZE + 2,
    #                                                            2 * MAX_SIZE)
    ...
    training_input = inputs[:int(len(inputs) * .90)]
    test_input = inputs[int(len(inputs) * .90):len(inputs)]

    training_output = outputs[:int(len(outputs) * .90)]
    test_output = outputs[int(len(outputs) * .90):len(outputs)]
    ...
    model = choose_model(args.model_name, n_hidden, vocab_size, n_layers,
                         memory_size, memory_dim)
    # Learning rate
    ...
    if args.load_path == '':
        ...
        for epoch in range(1, epoch_num + 1):
            ...
            for i in range(TRAINING_SIZE):
                ...
                # TODO: Currently outputs a char per char
                # We don't like this, we want all input before any output
                for j in range(len_input):
                    output, hidden, memory = model(
                        Dyck.lineToTensor(training_input[i][j]).to(device),
                        hidden, memory, temp)
                ...
                if np.all(np.equal(out_np, target_np)) and (
                        out_np.flatten() == target_np.flatten()).all():
                    counter += 1

                # At the end of the epoch, append our total loss and "moving" accuracy
                if i == TRAINING_SIZE - 1:
                    print('* Moving Accuracy: {}%'.format(
                        float(counter) / TRAINING_SIZE))
            ...
            if save_path == '':
                save_path = 'models/{}_model_weights.pth'.format(
                    args.model_name)
